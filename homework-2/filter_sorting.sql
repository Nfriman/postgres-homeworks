...
SELECT * FROM orders WHERE ship_country = 'Germany' OR ship_country = 'France' OR ship_country = 'Spain';
...
SELECT MIN(unit_price), max(unit_price) FROM products WHERE discontinued IN ('0');
...
SELECT MIN(unit_price), max(unit_price) FROM products WHERE discontinued IN ('0') AND units_in_stock>20 ;